/*
 * @Author: star-cs
 * @Date: 2025-07-22 22:18:13
 * @LastEditTime: 2025-07-22 22:58:57
 * @FilePath: /TMMS-SERVER/tmms/network/base/socketopt.cpp
 * @Description:
 */
...
InetAddress::ptr SocketOpt::GetLocalAddr() // 本地地址
{
    struct sockaddr_in6 addr_in;
...
/// @param saddr
/// @return 一个InetAddress的智能指针
InetAddress::ptr SocketOpt::GetPeerAddr() // 远端地址
{
    struct sockaddr_in6 addr_in;
    socklen_t           len = sizeof(struct sockaddr_in6);
    // 这个函数获取远端的地址和地址长度
...
void SocketOpt::SetTcpNoDelay(bool on)
{
    ...
    // TCP_NODELAY指定是否禁用Nagle算法的选项名称。当启用此选项（设置为
    // 1）时，它禁用Nagle算法，这意味着数据会尽快发送而不等待填满TCP数据包。
