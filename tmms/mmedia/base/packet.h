...
#pragma pack(push, 1) // 保存字节对齐状态
                      // 因为编译器会自动对齐类字节，所以这里设置成1字节对齐节省空间
class Packet
{
public:
    ...
    bool IsAudio() const { return type_ == kPacketTypeAudio; } // 音频没有其他属性，直接判断
    bool IsKeyFrame() const
    { // 只有视频有关键字
        return ((type_ & kPacketTypeVideo) == kPacketTypeVideo && (type_ & kPacketTypeKeyFrame) == kPacketTypeKeyFrame);
    }
    ...
    void         SetPacketSize(size_t len) { size_ = len; }
    void         UpdatePacketSize(size_t len) { size_ += len; }
    ...
    void SetExt(const std::shared_ptr<void>& ext) { ext_ = ext; }
