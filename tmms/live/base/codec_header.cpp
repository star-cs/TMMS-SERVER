...
{
    start_timestamp_ = base::TTime::NowMS();
}
CodecHeader::~CodecHeader()
{
}

PacketPtr CodecHeader::Meta(int idx) // 当前最新的包
{
    // 如果是第一个，直接返回
    if (idx <= 0)
    {
...
void CodecHeader::SaveMeta(const PacketPtr& packet)
{
    ...
    LIVE_TRACE(
        "save meta, meta version:{} size:{} elapse:{} ms",
        meta_version_,
        packet->PacketSize(),
        base::TTime::NowMS() - start_timestamp_);
}
...
void CodecHeader::SaveAudioHeader(const PacketPtr& packet)
{
    ...
    LIVE_TRACE(
        "save audio, audio version:{} size:{} elapse:{} ms",
        audio_version_,
        packet->PacketSize(),
        base::TTime::NowMS() - start_timestamp_);
}
...
void CodecHeader::SaveVideoHeader(const PacketPtr& packet)
{
    ...
    LIVE_TRACE(
        "save video, video version:{} size:{} elapse:{} ms",
        video_version_,
        packet->PacketSize(),
        base::TTime::NowMS() - start_timestamp_);
}
